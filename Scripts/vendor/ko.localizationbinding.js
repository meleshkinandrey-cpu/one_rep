...
define(["knockout"], function (ko) {
    ...
    update: function (element, valueAccessor, allBindingsAccessor, viewModel, context) {
        ...
        for (var attr in json) {
            (function (attr) { // Замыкание, чтобы избежать проблем с циклом
                var key = json[attr];
                var text = getLocalizedText(key);

                // Специальная обработка для placeholder и ключа 'UserLogin'
                if (attr === 'placeholder' && key === 'UserLogin') {
                    text = 'STEPCON\\' + text;
                }
        ...
        function getLocalizedText(binding) {
            if (typeof resourceArrayFromServer === "undefined") {
                throw "ko.localationbinding.getLocalizedText: resourceArrayFromServer object is not defined";
            }
            ...
            var key = binding.key;
            var item = resourceArrayFromServer[key];
            if (!item) {
