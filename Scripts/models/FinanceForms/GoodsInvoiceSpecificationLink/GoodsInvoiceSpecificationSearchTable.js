define(['knockout', 'jquery', 'ajax', 'models/Table/TableParts', 'dateTimeControl', 'comboBox'], function (ko, $, ajaxLib, tpLib, dtLib) {
    var module = {
        ViewModel: function ($regionTable) {
            ...
            self.OnSelectedRow = null;//set in GoodsInvoiceSpecification.js
            self.OnDeselectedRow = null;//set in GoodsInvoiceSpecification.js
            self.CheckRowSelection = null;//set in GoodsInvoiceSpecification.js
            self.selectedObjects = null;//set in GoodsInvoiceSpecification.js
            //
            ...
            self.SelectAllClick = function (item, event) {
                ...
                //
                var row = self.rowList()[0];
                self.typeID = checking ? row.OperationInfo.RowObject.ProductCatalogTypeID : null;
                if (!checking)
                    self.selectedObjects([]);
                //
                var $reloaded = self.Reload();
                //
                $.when($reloaded).done(function () {
                    ...
                    //
                    var checkbox = $regionTable.find('.b-content-table__th-checkbox')[0];
            ...
            self.RowSelectedChanged = function (row, isAdd) {
                ...
                if (isAdd && self.OnSelectedRow) {
                    ...
                    self.typeID = row.OperationInfo.RowObject.ProductCatalogTypeID;
                    self.Reload();
                }
                else if (!isAdd && self.OnDeselectedRow) {
                    self.OnDeselectedRow([row]);
                    //
                    var selectedObjs = self.selectedObjects;
                    if (selectedObjs() && selectedObjs().length != 0)
                        self.typeID = selectedObjs()[0].PurchaseSpecification.ProductCatalogTypeID;
                    else
                        self.typeID = null;
                    //
                    self.Reload();
            ...
            self.Load = function () {
                ...
                var requestInfo = {
                    ...
                    TypeID: self.typeID,
                    ModelsID: self.modelsID ? self.modelsID : [],
                    VendorsID: null,
                    LocationID: null,
                    LocationClassID: null,
                    ParentObjectID: self.workOrderID,
                    Mode: self.mode,
                    UsedPurchaseSpecificationList: self.usedIDList,
                };
                self.ajaxControl.Ajax($regionTable,
                    {
                        ...
                        method: 'POST',
                        data: requestInfo,
                        url: 'finApi/GetPurchaseSpecificationSearchListForTable'
                    },
                    function (newVal) {
