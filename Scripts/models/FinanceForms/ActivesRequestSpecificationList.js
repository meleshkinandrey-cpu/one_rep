define(['knockout', 'jquery', 'ajax', 'models/FinanceForms/ActivesRequestSpecification', 'ui_controls/ListView/ko.ListView.Cells', 'ui_controls/ListView/ko.ListView.Helpers', 'ui_controls/ListView/ko.ListView', 'ui_controls/ContextMenu/ko.ContextMenu'], function (ko, $, ajaxLib, specLib, m_cells, m_helpers) {
    var module = {
        ...
        LinkList: function (ko_object, objectClassID, ajaxSelector, readOnly_object, canEdit_object, $region) {
            ...
            {//дергается извне
                ...
                self.CheckData = function () {//функция загрузки списка (грузится только раз).
                    if (!self.isLoaded()) {
                        $.when(operationIsGrantedD(817), operationIsGrantedD(818), operationIsGrantedD(816), operationIsGrantedD(819), operationIsGrantedD(211), operationIsGrantedD(872)).
                            done(function (ars_add, ars_delete, ars_properties, ars_update, ps_propertiesContract, ps_propertiesAgreement) {
                                self.add_operation(ars_add);
                                self.delete_operation(ars_delete);
                                self.properties_operation(ars_properties);
                                self.propertiesContract_operation(ps_propertiesContract);
                                self.propertiesAgreement_operation(ps_propertiesAgreement);
                                self.update_operation(ars_update);
                            });
            ...
            {//строка состояния
                ...
                self.LoadNDSTotalValues = function () {
                    ...
                    self.ajaxControlTotalValues.Ajax($(ajaxSelector),
                    {
                        dataType: "json",
                        method: 'GET',
                        url: 'finApi/GetActivesRequestSpecificationNDSTotalValues?' + $.param(param)
                    },
                    function (newVal) {
            ...
            {//ko.listView
                {//events of listView
                    self.viewName = 'ActivesRequestSpecification';
                    self.listView = null;
                    //
                ...
                {//identification
                    ...
                    self.getObjectClassID = function (obj) {
                        return 380; //OBJ_ActivesRequestSpecification
                    };
                }
                //
                {//geting data             
                        ...
                        self.ajaxControl.Ajax(null,
                            {
                                ...
                                method: 'POST',
                                data: requestInfo,
                                url: 'finApi/GetActivesRequestSpecificationListForTable'
                            },
                            function (newVal) {
                                if (newVal && newVal.Result === 0) {
                                    ...
                                    require(['sweetAlert'], function () {
                                        swal(getTextResource('ErrorCaption'), getTextResource('NullParamsError') + '\n[ActivesRequestSpecificationList.js getObjectList]', 'error');
                                    });
                                }
                ...
                {//open object form
                    self.showObjectForm = function (classID, id) {
                        ...
                        self.ajaxControl.Ajax($(ajaxSelector),
                        {
                            ...
                            method: 'GET',
                            data: data,
                            url: 'finApi/GetActivesRequestSpecification'
                        },
                        function (newVal) {
                            if (newVal && newVal.Result === 0) {
                    ...
                    self.DoShowObjectForm = function (specification) {//отображает форму элемента списка                                    
                        ...
                        require(['financeForms'], function (module) {
                            ...
                            //
                            fh.ShowActivesRequestSpecification(ars, computedCanEdit, function (newData) {
                                if (!newData)
                                    return;
                                //
                                var data = newData;
                                    ...
                                    function (answer) {
                                        if (answer && answer.Response) {
                                            ...
                                            if (result === 0) {
                                                ...
                                                require(['sweetAlert'], function () {
                                                    swal(getTextResource('ErrorCaption'), getTextResource('NullParamsError') + '\n[FinanceForms.ActivesRequestSpecificationList.js, DoShowObjectForm]', 'error');
                                                });
                                            else if (result === 2)
                                                require(['sweetAlert'], function () {
                                                    swal(getTextResource('ErrorCaption'), getTextResource('BadParamsError') + '\n[FinanceForms.ActivesRequestSpecificationList.js, DoShowObjectForm]', 'error');
            ...
            {//operations
                                ...
                                self.ajaxControl.Ajax($(ajaxSelector),
                                    {
                                        ...
                                        method: 'POST',
                                        data: data,
                                        url: 'finApi/AddActivesRequestSpecificationList'
                                    },
                                    function (model) {
                                        if (model.Result === 0) {
                                            ...
                                            if (model.Result === 1) {
                                                require(['sweetAlert'], function () {
                                                    swal(getTextResource('SaveError'), getTextResource('NullParamsError') + '\n[FinanceForms.ActivesRequestSpecificationList.js, add_operationClick]', 'error');
                                                });
                                            }
                ...
                self.propertiesObjectMenuItem = function (contextMenu) {
                    ...
                    var isVisible = function () {
                        ...
                        if (items.length != 1)
                            return false;
                        var classID = items[0].BLL.ProductCatalogModelClassID;
                        return ((classID == 115 && self.propertiesContract_operation()) || (classID == 386 && self.propertiesAgreement_operation())) && items[0].BLL.ProductCatalogModelID != null;
                    };
                    var action = function () {
                        if (self.getSelectedItems().length != 1)
                            return false;
                        //
                        var selected = self.getSelectedItems()[0];
                        var id = selected.BLL.ProductCatalogModelID;
                        var classID = selected.BLL.ProductCatalogModelClassID;
                        //     
                        showSpinner();
                ...
                self.removeMenuItem = function (contextMenu) {
                    ...
                    var action = function () {
                        ...
                        require(['sweetAlert'], function () {
                            ...
                            swal({
                                title: getTextResource('ActivesRequestSpecificationOperationCaption'),
                                text: getTextResource('ActivesRequestSpecificationDeleteQuestion') + ' ' + nameList,
                                    ...
                                    self.ajaxControl.Ajax($(ajaxSelector),
                                        {
                                            ...
                                            method: 'POST',
                                            data: data,
                                            url: 'finApi/RemoveActivesRequestSpecification'
                                        },
                                        function (Result) {
                                            if (Result === 0) {
