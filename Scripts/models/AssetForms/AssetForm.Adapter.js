define(['knockout', 'jquery', 'models/SDForms/SDForm.User'], function (ko, $, userLib) {
    var module = {
        Adapter: function (parent, obj) {//NetworkDeviceForm.js, adapter
            var self = this;
            //
            if (obj.ID)
            ...
            else self.BuildingName = ko.observable('');
            //
            if (obj.RoomID)
            ...
            //
            if (obj.Integrated)
                self.Integrated = ko.observable(obj.Integrated);
            else self.Integrated = ko.observable(false);
            //
            self.ShowState = ko.computed(function () {
                return !self.IsLogical() && !self.Integrated() && self.LifeCycleStateName();
            });
            //
            ...
            //
            if (!self.IsLogical() && !self.Integrated())
